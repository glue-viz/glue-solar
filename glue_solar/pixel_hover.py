...
__all__ = ['HoverExtractionTool']


@viewer_tool
class HoverExtractionTool(ToolbarModeBase):
    """
    Create a derived dataset corresponding to the selected pixel.
    """

    icon = "glue_cross"
    tool_id = 'solar:hover_extraction'
    action_text = 'Pixel'
    tool_tip = 'Show data for a single pixel based on mouse location'
    status_tip = 'Mouse over to update the extracted dataset in real time'

    _pressed = False

    def __init__(self, *args, **kwargs):
        super(HoverExtractionTool, self).__init__(*args, **kwargs)
        self._move_callback = self._extract_pixel
        self._press_callback = self._on_press
        self._release_callback = self._on_release
        self._derived = None

        self._line_x = self.viewer.axes.axvline(0, color='blue')
        self._line_x.set_visible(False)

        self._line_y = self.viewer.axes.axhline(0, color='blue')
