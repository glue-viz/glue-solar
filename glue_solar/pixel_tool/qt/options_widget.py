...
class PixelToolOptionsWidget(QtWidgets.QWidget):
    ...
    def _on_attribute_change(self, *args):
        if (
            self.viewer_state.reference_data is None
        ...
        world_warning = (
            len(
                dependent_axes(
                    self.viewer_state.reference_data.coords,
                    self.viewer_state.x_att_pixel.axis,
                )
            )
            > 1
        )
