...
from context_chk import aia_context, fov_polygon, goes_xrs, link_hpc
...
def test_aia_context_from_local_map(monkeypatch, tmp_path, irispy_test_files):
    ...
    sunpy.map.Map(np.random.default_rng(0).random((800, 800)), hdr).save(tmp_path / "aia.fits")
    monkeypatch.setattr(
        context, "Fido", types.SimpleNamespace(search=lambda *a: None, fetch=lambda *a, **k: _results(tmp_path / "aia.fits"))
    )
