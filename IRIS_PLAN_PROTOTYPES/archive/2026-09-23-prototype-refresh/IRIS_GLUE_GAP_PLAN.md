...
The agreed target is **IRIS-first CRISPEX functionality**. This review
corrects the plan and indexes prototype evidence; it does not implement
the remaining features. CI status is outside this validation's scope.
...
in the [prototype index](IRIS_PLAN_PROTOTYPES/README.md#validation-2026-09-22).
...
the browser branch from before #52/#53: no `tools.py`, no SJI `Time`
component (raster/stack time components do exist), no NaN layer artist,
and a `setup()` that installs
`glue_patches`. Port `link_hpc` (`loaders/iris.py`), the `link_iris` menu
action, `glue_patches.py` and `tests/test_linking.py` onto current `main`;
do not copy the package over it.
...
prototype installs WCS/Quantity/style patches only; it does not wire the
browser to `load_data`. The standalone
[session acceptance tests](IRIS_PLAN_PROTOTYPES/wp3chk_test_sessions.py)
target a fully ported implementation and fail on unmodified `main`.
...
- [ ] Use WP1's actual helper contract. The datetime64 `Time` component
  ...
  prototype's `cube_times`/`add_time` duplicates. Add decimation-aware slit
  positions, then the per-frame slit subset, nearest index links, whisker
  preset, sync callbacks and browser quicklook. Replace the prototype's
  inferred power-of-ten slit scaling with explicit fixture decimation or
  validated pixel-coordinate conversion; an off-image slit is not evidence
  of decimation in a real observation.
- [ ] Use a data-only slit subset, not a collection-wide editable subset
  ...
  tie and maximum-offset behavior; the prototype clamps out-of-range times
  to an endpoint and matches observations by OBSID alone.
...
- [ ] Add the selected-location time–wavelength panel and light curve with
  ...
  close; the prototype wraps `app.new_data_viewer` and retains states.
...
[proposed production tests](IRIS_PLAN_PROTOTYPES/wp7chk/test_context.py).
Those tests import the nonexistent production `sources.context` module.
The runnable scratch version is
[test_from_brief.py](IRIS_PLAN_PROTOTYPES/wp7chk/test_from_brief.py), using
`context_chk.py`; current `.venv` collection is blocked by missing SunPy
timeseries extras. This is not a passing current validation.
...
For the progress-value correction, use
[ProtoImporterFixed](IRIS_PLAN_PROTOTYPES/wp8_chk_proto_fix.py): the primary
dialog prototype still omits `progress.setValue(0)` after entering busy
mode, even though its eight lifecycle/filter tests pass.
