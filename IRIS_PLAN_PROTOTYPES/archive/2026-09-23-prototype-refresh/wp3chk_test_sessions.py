"""Exact glue_solar/tests/test_sessions.py that passed (33 passed) in the checker's worktree on 2026-09-02."""
import json
import shutil
...
def test_sji_session_restores_gwcs_meta_and_colormap(irispy_test_files):
    ...
    assert len(restored.meta) == 148
    assert restored.style.preferred_cmap.name == "irissji1330"
    style = _style_record(s, original.label)
    assert style["_type"] == "glue_solar.sources.loaders.iris.SolarVisualAttributes"
