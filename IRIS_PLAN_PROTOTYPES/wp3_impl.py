"""WP3 session experiment: importing installs scoped WCS, Quantity and solar-style support.

Call install_browser_factories() explicitly to log browser inputs for file-based sessions.
No production files are changed. Keep this module importable when restoring its sessions.
"""
import base64
import io
import os
...
# Keep the unwrapped readers so logged factories do not recurse.
_image_data = L.image_data
_raster_data = L.raster_data
...
def read_iris_rasters(path, files=None, windows=None, stack=False):
    ...
    return _raster_data(files, windows, stack)


def read_iris_image(path):
    return _image_data(path)


def _logged_image(path):
    return load_data(str(path), factory=read_iris_image)


def _logged_rasters(files, windows=None, stack=False):
    files = [os.path.abspath(path) for path in files]
    folder = os.path.dirname(files[0])
    return as_list(load_data(
        files[0], factory=read_iris_rasters,
        files=[os.path.relpath(path, folder) for path in files], windows=windows, stack=stack,
    ))


def install_browser_factories():
    """Opt the browser into file-backed sessions within this process."""
    L.image_data = _logged_image
    L.raster_data = _logged_rasters
