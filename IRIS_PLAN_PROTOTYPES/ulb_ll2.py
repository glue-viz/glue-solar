...
class LineListState(LayerState):
    @property
    def attribute(self):
        return self.layer.id["wavelength"]
...
lw = v._view.layout_style_widgets
P("style widgets created for:", [type(k).__name__ for k in lw])
# now the same with a subclass-aware key
...
v2 = app.new_data_viewer(ProfileViewer, data=d); v2.state.x_att = v.state.x_att; v2.add_data(lines)
P("v2 style widgets:", [type(k).__name__ for k in v2._view.layout_style_widgets])
# toggle a layer property -> _layers_changed second branch (state.py:266) runs
v2.layers[1].state.visible = False; v2.layers[1].state.visible = True
P("visible toggle on line-list layer OK")
# 4D stack meta type
from glue_solar.sources.loaders.stack_spectrograms import stack_spectrograms
import inspect; src = inspect.getsource(stack_spectrograms)
P("stack meta is plain dict:", "meta=dict(" in src)
P("DONE2")
